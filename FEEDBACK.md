...
The VERIFY + SENDER frame pattern is flexible. Self-pay (1 VERIFY), sponsored (2 VERIFY), deploy (DEFAULT + VERIFY) all compose naturally. Adding a paymaster is just prepending another VERIFY frame with a different approval scope.
...
The spec defines frame modes (VERIFY, SENDER, DEFAULT) generically, but EOA "default code" gives them completely different data semantics:
...
These are different encoding and signing protocols that happen to share the same frame structure. Any implementer building tooling has to support both paths, and the spec doesn't clearly delineate them as two distinct modes. It reads like one transaction format but it's actually two.

**Suggestion:** The spec should name these as two sub-protocols (or at minimum, have a dedicated "Default Code Behavior" section clearly separated from the smart account flow). Implementers shouldn't have to discover the EOA/SA split by reading the default code pseudocode.

### 2. No `value` field in the frame structure

Frames have `[mode, target, gas_limit, data]`. There's no `value` field.
...
Every other Ethereum transaction type has a value field (legacy, EIP-1559, EIP-4844). Frames are the unit of execution in 8141, analogous to a transaction's call. Omitting value means sending ETH requires account-type-specific workarounds.
...
**Suggestion:** Add a `value` field to the frame structure: `[mode, target, value, gas_limit, data]`. This makes ETH transfers explicit at the wire format level and eliminates the need for EOA default code to define its own subcall encoding just to express value.
...
Same field, different meanings. You can't look at a serialized frame transaction and know what the SENDER frame data means without knowing the account type. Debuggers, indexers, and block explorers all need to resolve the account type before they can decode SENDER frames.
...
This packing creates some design tension:

**APPROVE is not restricted to VERIFY frames.** The spec does not restrict the APPROVE opcode to VERIFY mode; it's callable from SENDER and DEFAULT frames too. This means the entire VERIFY/SENDER separation is a convention, not an invariant. A SENDER frame can call APPROVE as a side effect of execution. A DEFAULT frame can approve during deployment. Approval state can be mutated at any point in the frame sequence.
...
**The scope bitmask does work well as a bitmask.** EXECUTION=1, PAYMENT=2, BOTH=3. `BOTH = EXECUTION | PAYMENT`. The runtime can check `(approved_flags & required_scope) == required_scope`. No branching.
...
The issues above (especially #6) point toward a simpler design. [0xrcinus's post on Ethereum Magicians](https://ethereum-magicians.org/t/eip-8141-frame-transaction/27617/104) proposes simplifications that match our implementation experience.

### Two modes, not three

**Current:** DEFAULT (0), VERIFY (1), SENDER (2). DEFAULT and SENDER are both "execute code" but with different callers (ENTRY_POINT vs sender).
...
**How this would have helped us:** Our SDK's `buildFrameTransaction` has to construct DEFAULT frames for deploys, VERIFY frames for signatures, and SENDER frames for calls. With two modes, deploy frames and call frames are both EXECUTE; the only question is whether they come before or after VERIFY. The builder gets simpler and the mode enum maps directly to "are you validating or executing?"
...
**Current:** APPROVE takes a scope operand (EXECUTION=1, PAYMENT=2, BOTH=3), constrained by 2 bits in the mode field. Ordering rules: PAYMENT requires sender already approved. Four enum values, complex interactions, no room for extension in the bitfield.
...
The identity of the caller determines the semantics: sender approves execution + payment, non-sender approves payment only. In practice you only ever have one or two VERIFY frames (sender and optionally payer). The scope bits add complexity for cases that don't arise in real usage.
...
**How this would have helped us:** Our current atomic batch handling is fragile:
- The flag means "I'm atomic *with the next frame*", a forward reference
...
With batch flags, this requires careful ordering and correct flag placement on each frame. With groups, you just label each frame. The SDK builder and validation both get simpler, and non-contiguous atomicity is possible.
...
The mode field goes from 3 concerns packed into bits to 2 (mode + group). APPROVE goes from parameterized-with-scope and callable-anywhere to no params, VERIFY-only, identity-determined. Atomicity goes from forward-linking chain to labeling.
...
- The `signature_type` byte in EOA VERIFY data (0x00 = ECDSA, 0x01 = P256) is forward-looking. But it's only defined for default code; smart accounts don't use it. Two signature format conventions in one spec.
...
2. Approval scope bits add complexity that caller identity can determine more simply.
...
The simplifications proposed by 0xrcinus on Ethereum Magicians (two modes, no scope bits, group-based atomicity) would have eliminated real implementation complexity in our SDK.
