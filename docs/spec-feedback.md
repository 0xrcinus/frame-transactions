...
VERIFY frame data gets zeroed before hashing, so both sender and paymaster compute the same sig hash regardless of signing order. No coordination needed. This was easy to implement and easy to reason about.
...
This is the big one. The spec defines frame modes (VERIFY, SENDER, DEFAULT) generically, but EOA "default code" gives them completely different data semantics:
...
These aren't minor variations — they're fundamentally different encoding and signing protocols using the same frame structure. Any implementer building tooling has to support both paths, and the spec doesn't clearly delineate them as two distinct modes. It reads like one transaction format but it's actually two.
...
- **Smart accounts** calling payable functions (WETH deposits, NFT mints with price, liquidity provision) need to forward ETH. Without a frame-level value, every smart account contract must implement its own `execute(target, value, data)` method to handle value forwarding. That's execution logic that should live at the protocol level, not duplicated across every smart account implementation. The whole point of frames is to move execution semantics out of account contracts and into the transaction format — but omitting value forces the most basic execution semantic (sending ETH) right back into account code.

Every other Ethereum transaction type has a value field. Legacy, EIP-1559, EIP-4844 — they all have it. Frames are the unit of execution in 8141, analogous to a transaction's call. Omitting value makes the most basic Ethereum operation (sending ETH) a second-class citizen that requires account-type-specific workarounds.
...
**Suggestion:** Add a `value` field to the frame structure: `[mode, target, value, gas_limit, data]`. This makes ETH transfers explicit at the wire format level, unifies behavior across account types, and eliminates the need for EOA default code to define its own subcall encoding just to express value.
...
Same field, different meanings. You can't look at a serialized frame transaction and know what the SENDER frame data means without knowing the account type. This makes debugging, indexing, and block explorers harder — they need to resolve the account type before they can decode SENDER frames.
...
The sig hash is computed the same way for both account types, which is good. But the *signing method* diverges in a way that has practical consequences for tooling:
...
- **EOA default code** calls raw `ecrecover(sigHash, v, r, s)` — no prefix. The signer must sign the bare hash.
...
This is compact, but it creates some design tension:

**APPROVE is not restricted to VERIFY frames.** This is the most surprising finding. The spec does not restrict the APPROVE opcode to VERIFY mode — it's callable from SENDER and DEFAULT frames too. This means the entire VERIFY/SENDER separation is a convention, not an invariant. A SENDER frame can call APPROVE as a side effect of execution. A DEFAULT frame can approve during deployment. Approval state can be mutated at any point in the frame sequence.
...
**No room for extension.** 2 bits, 4 values, all used. If a future use case needs a third approval dimension (e.g., storage access, cross-chain relay), the scope field has to grow. This isn't a problem today, but the tight packing means the mode field layout would need to change.

**The scope bitmask is elegant in one way.** EXECUTION=1, PAYMENT=2, BOTH=3. That's `BOTH = EXECUTION | PAYMENT` as a bitmask. The runtime can check `(approved_flags & required_scope) == required_scope`. Simple, no branching. This is good.

**Suggestion:** Either restrict APPROVE to VERIFY frames at the opcode level (making the validation/execution separation an invariant), or clearly document that any frame can approve and explain the interaction with scope bits. Also define a static validation rule that VERIFY frames with scope PAYMENT (0x2) must be preceded by a VERIFY frame with scope EXECUTION (0x1) or BOTH (0x3). Clarify whether non-applicable mode bits (scope on SENDER, atomic batch on VERIFY) must be zero.

## Simplification proposals

The issues above (especially #6) point toward a simpler design. [0xrcinus's post on Ethereum Magicians](https://ethereum-magicians.org/t/eip-8141-frame-transaction/27617/104) proposes three simplifications that we strongly agree with based on our implementation experience.
...
**How this would have helped us:** Our SDK's `buildFrameTransaction` has to construct DEFAULT frames for deploys, VERIFY frames for signatures, and SENDER frames for calls — three different frame creation paths. With two modes, deploy frames and call frames are both EXECUTE; the only question is whether they come before or after VERIFY. The builder gets simpler and the mode enum is a cleaner concept: "are you validating or executing?"
...
**Current:** APPROVE takes a scope operand (EXECUTION=1, PAYMENT=2, BOTH=3), constrained by 2 bits in the mode field. Ordering rules: PAYMENT requires sender already approved. Four enum values, complex interactions, no room for extension.
...
- The flag means "I'm atomic *with the next frame*" — it's a forward reference
...
During live testing against the ethrex demo node, we confirmed that **value transfers in EOA SENDER frames silently fail**. The frame receipt shows `status: 0x1` (success) and gas is consumed, but ETH never arrives at the recipient. The sender balance doesn't decrease by the value amount either.

**Root cause** (in `crates/vm/levm/src/opcode_handlers/frame_tx.rs`): The `execute_default_sender` function creates a `CallFrame` with `should_transfer_value = !call.value.is_zero()` (line 639) and `msg_value = call.value` (line 634), then calls `vm.run_execution()` directly. But `run_execution()` just enters the opcode loop — it never checks `should_transfer_value` or calls `self.transfer()`.
...
The core frame transaction design is sound — composable validation, clean sig hash elision, and typed transaction envelopes all work well. The main friction points are:
...
4. **Atomic batch flags are a fragile forward-linking mechanism** where group IDs would be cleaner.

The simplifications proposed by 0xrcinus on Ethereum Magicians (two modes, no scope bits, group-based atomicity) would have eliminated significant implementation complexity in our SDK. They address real friction we encountered, not theoretical concerns.
